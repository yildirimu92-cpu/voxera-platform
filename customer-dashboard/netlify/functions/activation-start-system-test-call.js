...
exports.handler = async (event) => {
  ...
  // Status-Check entfernt: Im neuen Flow kann der Testanruf jederzeit ausgelöst werden.
  // Status wird beim Trigger automatisch auf pending_test gesetzt um Polling-Logik zu unterstützen.
  if (String(customer.forwarding_status || '').toLowerCase() !== 'pending_test') {
    try {
      await sbAdmin
        .from('customers')
        .update({ forwarding_status: 'pending_test' })
        .eq('id', customer.id);
      customer.forwarding_status = 'pending_test';
    } catch (e) {
      console.warn('[activation-start-system-test-call] failed to set pending_test', e);
    }
  }

  // Auto-create activation session if missing (simplified flow)
  let testSessionStartedAt = toStr(customer.activation_started_at);
  if (!testSessionStartedAt) {
    const nowIso = new Date().toISOString();
    try {
      await sbAdmin
        .from('customers')
        .update({ activation_started_at: nowIso })
        .eq('id', customer.id);
      customer.activation_started_at = nowIso;
      testSessionStartedAt = nowIso;
      console.log('[activation-start-system-test-call] auto_created_session', { customer_id: customer.id, started_at: nowIso });
    } catch (e) {
      console.error('[activation-start-system-test-call] failed to set activation_started_at', e);
      return response(500, { error: 'Aktivierungssitzung konnte nicht gestartet werden.' });
    }
  }
  ...
  // Simplified flow: always make a fresh call when user explicitly requests it
  // Reset previous test state so a new outbound call is triggered
  if (existingCandidateCallId) {
    try {
      await sbAdmin
        ...
        .eq('id', customer.id);
      customer.activation_test_candidate_call_id = null;
      customer.activation_started_at = new Date().toISOString();
      testSessionStartedAt = customer.activation_started_at;
      console.log('[activation-start-system-test-call] reset_guard_for_fresh_call', { customer_id: customer.id });
    } catch(e) {
      console.warn('[activation-start-system-test-call] reset_guard_failed', e);
    }
  }
  // After reset above, existingCandidateCallId is null — this block is a safety fallback
