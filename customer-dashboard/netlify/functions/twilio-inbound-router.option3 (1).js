...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ─── Loop-Erkennung ──────────────────────────────────────────────────
    const isLoopCall = fromNormalized === toNormalized;
    ...
    // ─── KEIN provisionaler INSERT mehr ──────────────────────────────────
    // Make.com legt den Datensatz nach Anrufende über den ElevenLabs-Webhook an.
    // Damit gibt es nur einen Datensatz pro Anruf, keine Duplikate.
    //
    // Trade-off: Der Anruf erscheint im Customer Dashboard erst nach Anrufende
    // (typisch 30-90 Sek), nicht live während er läuft. Das ist für den
    // aktuellen Use-Case akzeptabel.
    //
    // TODO (vor Public Launch): saubere Korrelation via ElevenLabs Dynamic
    // Variables aufbauen, dann Live-Indikator wieder einführen.

    if (!customer?.id) {
      console.warn('[twilio-inbound-router] customer not found for incoming number', {
        callSid,
        to: toNormalized
      });
    }
