...
// Defensiv: matched auch wenn Identifier in ElevenLabs Whitespace oder
// abweichende Schreibweise hat (z.B. ' caller_name' statt 'caller_name').
function pickDC(dcResults, key) {
  if (!dcResults || typeof dcResults !== 'object') return null;

  // Direkter Match zuerst
  let entry = dcResults[key];

  // Fallback: case-insensitive + getrimmt
  if (entry == null) {
