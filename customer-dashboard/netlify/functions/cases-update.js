...
function isMissingManualTasksSchema(error) {
  ...
  const relationMentioned = /\bcustomer_tasks\b/.test(combined);
...
function isPriorityConstraintError(error) {
  ...
  const hint = String(error?.hint || '').toLowerCase();
  const combined = `${message} ${details} ${hint}`;
  return combined.includes('customer_tasks_priority_check')
    || (/\bcustomer_tasks\b/.test(combined) && /priority/.test(combined) && /violates check constraint/.test(combined));
}
...
exports.handler = async (event) => {
  ...
  const caller = await requireCustomerCaller({ event, sbUrl, sbAnonKey, sbAdmin, requireActiveContract: true, functionName: 'cases-update' });
