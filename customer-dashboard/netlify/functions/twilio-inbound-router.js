...
// ─── ÄNDERUNG 1: callSid als Parameter, wird an ElevenLabs durchgereicht ───
function twimlRedirectResponse(callSid) {
  const sid = toStringOrEmpty(callSid);
  const targetUrl = sid
    ? `${ELEVENLABS_INBOUND_URL}?CallSid=${encodeURIComponent(sid)}`
    : ELEVENLABS_INBOUND_URL;
...
exports.handler = async (event) => {
  ...
  // ─── ÄNDERUNG 2: callSid wird ab hier in alle Redirect-Aufrufe gegeben ───
  if (!callSid || !toRaw) {
    ...
    return twimlRedirectResponse(callSid);
  }
  ...
  // ─── ÄNDERUNG 3: callSid wird beim Erfolgs-Redirect mitgegeben ───
  return twimlRedirectResponse(callSid);
};
