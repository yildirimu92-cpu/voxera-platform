(function installCustomerSettingsPolish(root) {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #tab-hilfe .vx-page-header-title {
        color: #fff;
      ...
      #tab-hilfe .vx-page-header-subtitle {
        margin-top: 4px;
      ...
      #tab-hilfe .vx-settings-card {
        overflow: hidden;
        margin-bottom: 16px;
        border: 1px solid #dbe4ef;
        border-radius: 22px;
        background: #fff;
        box-shadow: 0 4px 16px rgba(15,35,71,.05);
      }

      #mehr-sub-profil .vx-settings-card-head,
      #tab-hilfe .vx-settings-card-head {
        padding: 20px 22px 0;
      }

      #mehr-sub-profil .vx-settings-card-title,
      #tab-hilfe .vx-settings-card-title {
        margin: 0;
        color: #0f172a;
        font-size: 19px;
        font-weight: 760;
      ...
      #tab-hilfe .vx-settings-card-meta {
        margin-top: 4px;
        color: #64748b;
        line-height: 1.5;
      }

      #mehr-sub-profil .vx-settings-card-body {
        padding: 18px 22px 22px;
      }

      #mehr-sub-profil .vx-settings-identity {
        display: flex;
        align-items: center;
        gap: 14px;
        padding: 20px 22px;
      ...
      #mehr-sub-profil .vx-settings-field :is(input, select) {
      ...
      #mehr-sub-profil .vx-settings-actions .btn {
        min-height: 48px;
        border-radius: 13px;
      }

      #tab-hilfe .vx-help-action-grid {
        display: grid;
      ...
      #tab-hilfe .vx-help-faq {
        margin: 0;
        padding: 20px 22px 22px;
      }

      #tab-hilfe .vx-help-faq details {
        padding: 16px 0;
        border-bottom: 1px solid #e5ebf2;
      }

      #tab-hilfe .vx-help-faq details:last-child {
        border-bottom: 0;
      }
      ...
      @media (max-width: 600px) {
        ...
        #tab-hilfe .vx-help-action small {
          grid-column: 2;
        }
      }
    `;
  ...
  function installEnterpriseUpgrade() {
    ...
    root.vxAboUpgrade = function openUpgradeWithEnterprise() {
      const currentPlan = String((root.customerMeta && root.customerMeta.plan) || '').trim().toLowerCase();
      const order = ['starter', 'business', 'professional'];
      const labels = { starter: 'Starter', business: 'Business', professional: 'Professional' };
      const prices = { business: 249, professional: 399 };
      const minutes = { business: 150, professional: 350 };
      ...
      order.forEach(function(plan, index) {
        ...
        options.push({
          value: plan,
          title: labels[plan],
          meta: 'CHF ' + prices[plan] + ' / Mt · ' + minutes[plan] + ' Min. inklusive'
        });
      });

      if (currentPlan === 'professional' || options.length === 0) {
        options.push({
          value: 'enterprise',
          title: 'Enterprise',
