(function (root, factory) {
  ...
  function getAudioErrorMessage(error) {
    ...
    if (code === 'session_missing' || code === 'access_token_missing' || status === 401) return 'Ihre Sitzung ist abgelaufen. Bitte melden Sie sich erneut an.';
  ...
  function createConversationAudioClient(options) {
    ...
    if (!urlApi || typeof urlApi.createObjectURL !== 'function' || typeof urlApi.revokeObjectURL !== 'function') throw new TypeError('urlApi with createObjectURL/revokeObjectURL is required');

    async function getAccessToken() {
      if (!authClient || !authClient.auth || typeof authClient.auth.getSession !== 'function') throw new ConversationAudioError('session_missing', 401);
    ...
    async function loadAudio(input) {
      ...
      const response = await fetchFn(`${CONVERSATION_AUDIO_ENDPOINT}?conversation_id=${encodeURIComponent(conversationId)}`, {
        method: 'GET',
        headers: { Authorization: `Bearer ${accessToken}`, Accept: 'audio/mpeg, audio/*, application/octet-stream' }
      });
      if (!response || !response.ok) throw new ConversationAudioError(`audio_request_${Number(response && response.status) || 502}`, Number(response && response.status) || 502);
  ...
  function installConversationAudioBridge(target) {
    ...
    function getAuthClient() { return typeof browser.getSupabaseAuthClient === 'function' ? browser.getSupabaseAuthClient() : (browser._sb || null); }
    function renderProtectedAudioPlaceholder(callOrFields, protectedUrl) {
      let conversationId = typeof browser.vxGetElevenLabsConversationId === 'function' ? String(browser.vxGetElevenLabsConversationId(callOrFields) || '').trim() : '';
      if (!conversationId && protectedUrl) {
        try { conversationId = String(new URL(String(protectedUrl), browser.location && browser.location.origin || 'https://dashboard.voxera.ch').searchParams.get('conversation_id') || '').trim(); } catch { conversationId = ''; }
      }
      if (!conversationId) return '';
      return '<div class="vx-transcript-card" id="vx-call-audio-card" data-conversation-id="' + escapeAttr(conversationId) + '"><div class="vx-transcript-head"><i class="ph-bold ph-waveform" aria-hidden="true"></i><div class="vx-transcript-title">Audioaufnahme</div><div class="vx-transcript-meta">ElevenLabs</div></div><div class="vx-transcript-empty">Audioaufnahme ist geschützt verfügbar.<br><button type="button" class="btn btn--secondary btn--sm" style="margin-top:10px;" onclick="vxTryLoadElevenLabsAudioFromDashboard(this)">Audio abrufen</button><div class="vx-audio-fetch-status" style="font-size:11px;color:var(--slate2);margin-top:8px;"></div></div></div>';
    }
    const originalRenderCallAudioCard = typeof browser.vxRenderCallAudioCardHtml === 'function' ? browser.vxRenderCallAudioCardHtml : null;
    if (originalRenderCallAudioCard) {
      browser.vxRenderCallAudioCardHtml = function secureRenderCallAudioCard(callOrFields) {
        const audioUrl = typeof browser.vxGetCallAudioUrl === 'function' ? browser.vxGetCallAudioUrl(callOrFields) : '';
    ...
    browser.vxTryLoadElevenLabsAudioFromDashboard = async function secureConversationAudioLoad(btn) {
      ...
      try {
        ...
        // bringt seine Kartenrahmung selbst mit, ein zusaetzliches
        // .vx-transcript-card plus Innenpolsterung ergaebe einen doppelten
        // Rahmen. Fuer geschuetzte Aufnahmen laeuft der Mount ausschliesslich
        // hier durch, deshalb muss die Struktur an beiden Stellen gleich sein.
        const nextCard = mountCardHtml(card, '<div id="vx-call-audio-card" class="vx-audio-host" data-vx-object-url="' + escapeAttr(activeObjectUrl) + '">' + browser.vxRenderModernAudioPlayerHtml(activeObjectUrl, 'Gerade sicher abgerufen') + '</div>');
  ...
  function scheduleConversationAudioBridge(target) {
    ...
    if (browser.document.readyState === 'loading') browser.document.addEventListener('DOMContentLoaded', install, { once: true });
    else install();
  }

  scheduleConversationAudioBridge(root);
  return { OFFER_BRAND_ASSET_PATH, OFFER_BRAND_ASSET_NAME, BRAND_LOGO_PATH, BRAND_FAVICON_PATH, ConversationAudioClient: { CONVERSATION_AUDIO_ENDPOINT, ConversationAudioError, createConversationAudioClient, getAudioErrorMessage, isProtectedConversationAudioUrl, installConversationAudioBridge } };
});
...
(function loadCustomerRuntimes(root) {
  ...
  [
    ...
    ['__voxeraCustomerCalendarSettingsLoaded', '/shared/customer-runtime-calendar-settings.js?v=20260808-1'],
    ['__voxeraCustomerOperationalUpdatesLoaded', '/shared/customer-runtime-operational-updates.js?v=20260808-1'],
    ['__voxeraCustomerAssistantProfileLoaded', '/shared/customer-runtime-assistant-profile.js?v=20260808-4'],
    ...
    ['__voxeraCustomerDesignFoundationLoaded', '/shared/customer-runtime-design-foundation.js?v=20260808-1']
  ].forEach(([flag, src]) => {
    if (root[flag]) return;
    root[flag] = true;
