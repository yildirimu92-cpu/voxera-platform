(function initVoxeraOperationalUpdates(root) {
  ...
  async function load(){setStatus('Aktuelle Änderungen werden geladen …','loading');try{state=await call({action:'list'});render();}catch(error){setStatus(error?.message||'Aktuelle Änderungen konnten nicht geladen werden.','error');}}
  ...
  function open(){if(!inject())return;render();load();}
