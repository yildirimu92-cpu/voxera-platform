(function initVoxeraOperationalUpdates(root) {
  ...
  const generalLabels = { normal:'Normal beantworten', mention:'Zusätzlich kurz auf die Betriebsinfo hinweisen', lead:'Betriebsinfo zuerst erwähnen' };
  ...
  function addStyle() {
    ...
    node.textContent = [
      '.vx-ops-layout{display:grid;grid-template-columns:minmax(0,1fr) minmax(340px,.8fr);gap:14px}',
      ...
      '.vx-ops-rules{display:grid;gap:5px;margin-top:10px;padding:10px;border-radius:10px;background:#f8fafc}',
      '.vx-ops-rule{display:grid;grid-template-columns:120px 1fr;gap:8px;font-size:11px;line-height:1.4}',
      '.vx-ops-rule b{color:#64748b}.vx-ops-rule span{color:#0d1f3c}',
      ...
      '.vx-ops-btn.secondary{background:#eef4ff;color:#1a6fe8}.vx-ops-btn.danger{background:#fef2f2;color:#b91c1c}.vx-ops-btn:disabled{opacity:.45;cursor:not-allowed}',
      '.vx-ops-form{display:grid;gap:13px;margin-top:14px}.vx-ops-field label{display:block;font-size:11px;font-weight:650;color:var(--slate2,#7a8599);margin-bottom:5px}',
      ...
      '.vx-ops-field textarea{min-height:82px;resize:vertical}.vx-ops-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}',
      '.vx-ops-section{border:.5px solid var(--line,#e4e8f0);border-radius:11px;padding:12px;background:#fbfcfe}.vx-ops-section-title{font-size:12px;font-weight:700;color:var(--ink,#0d1f3c);margin-bottom:8px}',
      '.vx-ops-summary{font-size:12px;line-height:1.55;color:#0d1f3c}.vx-ops-help{font-size:11px;line-height:1.45;color:var(--slate2,#7a8599);margin-top:5px}',
      '.vx-ops-preview{border-radius:11px;padding:12px;background:#f1f7ff;border:.5px solid #dbeafe}.vx-ops-preview strong{display:block;font-size:11px;color:#1d4ed8;margin-bottom:6px}.vx-ops-preview p{margin:0;font-size:12px;line-height:1.55;color:#0d1f3c}',
      '.vx-ops-details summary{cursor:pointer;font-size:12px;font-weight:650;color:#1a6fe8}.vx-ops-fixed{display:grid;gap:7px;margin-top:9px}.vx-ops-fixed-line{display:flex;gap:8px;align-items:flex-start;font-size:11px;line-height:1.45;color:#475569}.vx-ops-fixed-line i{color:#047857;margin-top:2px}',
      '.vx-ops-status{display:none;margin:0 0 12px;padding:9px 11px;border-radius:10px;font-size:12px;max-width:max-content}.vx-ops-status.loading{display:block;background:#eff6ff;color:#1d4ed8}.vx-ops-status.success{display:block;background:#ecfdf5;color:#047857}.vx-ops-status.warning{display:block;background:#fff7ed;color:#9a3412}.vx-ops-status.error{display:block;background:#fef2f2;color:#b91c1c}',
      '.vx-ops-empty{padding:24px 12px;text-align:center;color:var(--slate2,#7a8599);font-size:12px}.vx-ops-note{margin-top:12px;padding:11px 12px;border-radius:10px;background:#f8fafc;color:#64748b;font-size:11px;line-height:1.5}',
      '@media(max-width:820px){.vx-ops-layout,.vx-ops-grid{grid-template-columns:1fr}.vx-ops-rule{grid-template-columns:1fr}}'
    ].join('');
  ...
  function inject() {
    ...
    if (!document.getElementById('mehr-sub-betriebsinfos')) {
      ...
      page.id = 'mehr-sub-betriebsinfos'; page.style.display = 'none';
      ...
      tab.appendChild(page); page.querySelector('[data-ops-back]').addEventListener('click', back);
    }
    return true;
  }
  ...
  function itemHtml(item){ const status=phase(item); const canEdit=item.status!=='cancelled'&&status.key!=='expired'; const sync=item.sync_status==='failed'?' · Prompt-Sync ausstehend':''; const rules=parseBehavior(item.behavior); return '<div class="vx-ops-item '+esc(status.key)+'"><div class="vx-ops-head"><div><div class="vx-ops-title">'+esc(item.title)+'</div><div class="vx-ops-sub">'+esc(labels[item.type]||item.type)+'</div></div><span class="vx-ops-pill '+esc(status.key)+'">'+esc(status.label)+'</span></div><div class="vx-ops-message">'+esc(item.message)+'</div><div class="vx-ops-rules"><div class="vx-ops-rule"><b>Neue Termine</b><span>'+esc(rules.appointment||'Gemäss Standardlogik')+'</span></div><div class="vx-ops-rule"><b>Rückrufwünsche</b><span>'+esc(rules.callback||'Gemäss Standardlogik')+'</span></div><div class="vx-ops-rule"><b>Allgemeine Fragen</b><span>'+esc(rules.general||'Normal beantworten')+'</span></div><div class="vx-ops-rule"><b>Bestehende Termine</b><span>Absagen und Verschiebungen weiterhin bearbeiten</span></div><div class="vx-ops-rule"><b>Notfälle</b><span>Bestehende Notfall- und Sicherheitsregeln verwenden</span></div></div><div class="vx-ops-meta">'+esc(formatDate(item.starts_at))+' – '+esc(formatDate(item.ends_at))+esc(sync)+'</div>'+(canEdit?'<div class="vx-ops-actions"><button type="button" class="vx-ops-btn secondary" data-ops-edit="'+esc(item.id)+'">Bearbeiten</button><button type="button" class="vx-ops-btn danger" data-ops-cancel="'+esc(item.id)+'">Zurückziehen</button></div>':'')+'</div>'; }
  function option(value,label){ return '<option value="'+esc(value)+'">'+esc(label)+'</option>'; }

  function formHtml(){ return '<div class="vx-ops-form"><div class="vx-ops-field"><label>Was ist passiert?</label><select id="vx-ops-type">'+Object.entries(labels).map(([value,label])=>option(value,label)).join('')+'</select></div><div class="vx-ops-grid"><div class="vx-ops-field"><label id="vx-ops-start-label">Gültig ab</label><input id="vx-ops-start" type="datetime-local"></div><div class="vx-ops-field"><label id="vx-ops-end-label">Gültig bis</label><input id="vx-ops-end" type="datetime-local"></div></div><div class="vx-ops-section"><div class="vx-ops-section-title">Empfohlene Gesprächslogik</div><div class="vx-ops-summary" id="vx-ops-summary"></div><details class="vx-ops-details" id="vx-ops-rule-details"><summary>Einstellungen anpassen</summary><div style="display:grid;gap:11px;margin-top:11px"><div class="vx-ops-field"><label>Neue Termine</label><select id="vx-ops-appointment">'+option('after_end','Termine nach dem Zeitraum anbieten (empfohlen)')+option('available_only','Nur verfügbare Zeiten berücksichtigen')+option('collect','Keine Termine buchen, Rückrufanfrage aufnehmen')+option('blocked','Keine Termine buchen und keine Rückrufzusage machen')+option('normal','Normale Terminlogik beibehalten')+'</select></div><div class="vx-ops-field"><label>Rückrufwünsche</label><select id="vx-ops-callback">'+option('after_end','Aufnehmen und Rückruf nach dem Zeitraum ankündigen')+option('collect','Kontaktdaten aufnehmen')+option('normal','Normal aufnehmen')+option('temporary_contact','An temporäre Kontaktperson verweisen')+option('blocked','Keine Rückrufzusage machen')+'</select></div><div class="vx-ops-field"><label>Allgemeine Fragen</label><select id="vx-ops-general">'+option('normal','Normal beantworten')+option('mention','Zusätzlich kurz auf die Betriebsinfo hinweisen')+option('lead','Betriebsinfo zuerst erwähnen')+'</select></div></div></details><div class="vx-ops-fixed"><div class="vx-ops-fixed-line"><i class="ph-bold ph-check-circle"></i><span><strong>Bestehende Termine:</strong> Absagen und Verschiebungen werden weiterhin bearbeitet.</span></div><div class="vx-ops-fixed-line"><i class="ph-bold ph-shield-check"></i><span><strong>Notfälle:</strong> Bestehende Notfall- und Sicherheitsregeln bleiben aktiv.</span></div></div></div><div class="vx-ops-preview"><strong>So informiert Voxera Anrufende</strong><p id="vx-ops-preview"></p></div><details class="vx-ops-details"><summary>Text und erweiterte Anweisung anpassen</summary><div style="display:grid;gap:11px;margin-top:11px"><div class="vx-ops-field"><label>Information für Anrufende</label><textarea id="vx-ops-message" maxlength="500"></textarea></div><div class="vx-ops-field"><label>Zusätzliche KI-Anweisung (optional)</label><textarea id="vx-ops-extra" maxlength="300" placeholder="Nur für echte Ausnahmen verwenden."></textarea></div><div class="vx-ops-field"><label>Interner Titel</label><input id="vx-ops-title" maxlength="100"></div></div></details><div class="vx-ops-actions"><button type="button" class="vx-ops-btn" id="vx-ops-save"'+(busy?' disabled':'')+'>'+(editingId?'Änderung speichern':'Veröffentlichen')+'</button>'+(editingId?'<button type="button" class="vx-ops-btn secondary" id="vx-ops-reset">Abbrechen</button>':'')+'</div></div>'; }

  function render(){ const body=document.getElementById('vx-operational-page-body'); if(!body)return; const items=visibleItems(); const activeCount=items.filter((item)=>['active','scheduled'].includes(phase(item).key)).length; body.innerHTML='<div id="vx-ops-status" class="vx-ops-status" role="status" aria-live="polite"></div><div class="vx-ops-layout"><section class="vx-ops-card"><div class="vx-ops-title">Aktiv und geplant</div><div class="vx-ops-sub">Temporäre Regeln gelten nur im gewählten Zeitraum. Dauerhafte Unternehmensdaten bleiben unverändert.</div><div class="vx-ops-list">'+(items.length?items.map(itemHtml).join(''):'<div class="vx-ops-empty">Noch keine Betriebsinfo erfasst.</div>')+'</div></section><section class="vx-ops-card"><div class="vx-ops-title" id="vx-ops-form-title">'+(editingId?'Betriebsinfo bearbeiten':'Neue Betriebsinfo')+'</div><div class="vx-ops-sub">Wählen Sie die geschäftliche Situation. Voxera schlägt eine sichere Gesprächslogik vor.</div>'+formHtml()+'<div class="vx-ops-note">Temporäre Angaben dürfen keine dauerhaften Leistungen, Preise, Agenten-Funktionen oder Sicherheitsregeln überschreiben.</div></section></div>'; bind(); if(!editingId){setDefaults();applyPreset(document.getElementById('vx-ops-type')?.value||'closure',true);} const entrySub=document.getElementById('vx-operational-entry-sub'); if(entrySub) entrySub.textContent=activeCount?activeCount+' aktiv oder geplant':'Ferien, Abwesenheiten und Sonderzeiten'; }
  ...
  async function load(){setStatus('Betriebsinfos werden geladen …','loading');try{state=await call({action:'list'});render();}catch(error){setStatus(error?.message||'Betriebsinfos konnten nicht geladen werden.','error');}}
  ...
  async function save(){if(busy)return;const startValue=document.getElementById('vx-ops-start')?.value||'';const endValue=document.getElementById('vx-ops-end')?.value||'';const startDate=new Date(startValue);const endDate=new Date(endValue);if(!Number.isFinite(startDate.getTime())||!Number.isFinite(endDate.getTime())||endDate<=startDate){setStatus('Bitte einen gültigen Zeitraum wählen. Das Ende muss nach dem Beginn liegen.','error');return;}const update={type:document.getElementById('vx-ops-type')?.value,title:document.getElementById('vx-ops-title')?.value,message:document.getElementById('vx-ops-message')?.value,behavior:buildBehavior(),starts_at:startDate.toISOString(),ends_at:endDate.toISOString()};if(!update.title.trim()||!update.message.trim()){setStatus('Bitte Information und Zeitraum vollständig angeben.','error');return;}busy=true;setStatus('Betriebsinfo und Agenten-Prompt werden aktualisiert …','loading');try{state=await call({action:editingId?'update':'create',id:editingId||undefined,update});editingId='';render();if(state.sync_status==='failed')setStatus('Betriebsinfo gespeichert. Der Prompt-Sync wird als ausstehend markiert.','warning');else setStatus('✓ Gespeichert und mit dem Assistenten synchronisiert.','success');}catch(error){setStatus(error?.message||'Betriebsinfo konnte nicht gespeichert werden.','error');}finally{busy=false;}}
  async function cancel(id){if(busy||!root.confirm('Diese Betriebsinfo wirklich zurückziehen?'))return;busy=true;setStatus('Betriebsinfo wird zurückgezogen …','loading');try{state=await call({action:'cancel',id});editingId='';render();setStatus(state.sync_status==='failed'?'Betriebsinfo zurückgezogen. Prompt-Sync ist noch ausstehend.':'✓ Betriebsinfo wurde zurückgezogen.',state.sync_status==='failed'?'warning':'success');}catch(error){setStatus(error?.message||'Betriebsinfo konnte nicht zurückgezogen werden.','error');}finally{busy=false;}}
  function open(){if(!inject())return;document.getElementById('mehr-main').style.display='none';document.querySelectorAll('[id^="mehr-sub-"]').forEach((node)=>{node.style.display='none';});document.getElementById('mehr-sub-betriebsinfos').style.display='block';render();load();}
  function back(){document.querySelectorAll('[id^="mehr-sub-"]').forEach((node)=>{node.style.display='none';});document.getElementById('mehr-main').style.display='block';}
  root.vxOperationalUpdatesOpen=open;
  const install=()=>{if(!inject()){root.setTimeout(install,300);return;}};
