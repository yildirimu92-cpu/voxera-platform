...
create table if not exists public.telephony_numbers (
 ...
 customer_id uuid references public.customers(id) on delete set null, voice_url text,
...
create table if not exists public.telephony_number_assignment_audit (
 id uuid primary key default gen_random_uuid(), provider text not null default 'twilio',
 provider_number_sid text, phone_e164 text not null, customer_id uuid, previous_customer_id uuid,
...
create or replace function public.assign_twilio_number(p_customer_id uuid,p_number_sid text,p_phone_e164 text,p_actor_id text default null,p_voice_url text default null)
returns setof public.telephony_numbers language plpgsql security definer set search_path=public as $$
declare v_other uuid;v_previous_customer uuid;v_previous_number text;v_now timestamptz:=now();
begin
...
revoke all on function public.assign_twilio_number(uuid,text,text,text,text) from public;
revoke all on function public.assign_twilio_number(uuid,text,text,text,text) from anon;
revoke all on function public.assign_twilio_number(uuid,text,text,text,text) from authenticated;
grant execute on function public.assign_twilio_number(uuid,text,text,text,text) to service_role;
commit;
