-- Fix: admin_create_standalone_credit_note_v1 schrieb issued_at = now()
-- statt des vom Admin gewaehlten Ausstellungsdatums (v_issued_on).
--
-- issued_at ist die einzige der beiden Datumsspalten, die irgendein Leser
-- verwendet: Kunden-Dashboard (Liste/Sortierung), swiss-qr-bill.js (Datum auf
-- dem PDF), invoice-mail-dispatch.js (issued_on in der E-Mail an den Kunden).
-- invoice_date erhielt v_issued_on bereits korrekt, wird aber von keinem
-- Codepfad im Repo gelesen -- der Trigger trg_invoices_compat_normalize_v1
-- (2026-04-12_invoices_compatibility_bridge.sql) synchronisiert beide Felder
-- nur, wenn eines davon NULL ist; hier waren beide explizit gesetzt, also
-- griff er nicht.
--
-- Wirkung des Fehlers: eine Kulanzgutschrift mit einem vom Admin
-- zurueckdatierten Ausstellungsdatum haette auf dem Kunden-PDF, in der
-- E-Mail und im Dashboard trotzdem das Erstellungsdatum getragen. Stand
-- 11.08.2026 folgenlos, weil noch keine einzige credit_note-Zeile in
-- Produktion existiert (geprueft gegen ulcofbgrovgcvowdjrge) -- deshalb
-- jetzt der guenstigste Zeitpunkt fuer die Korrektur.
--
-- Einzige Aenderung gegenueber 2026-08-10_standalone_credit_notes.sql: der
-- issued_at-Wert im insert. invoice_date, Rechte und Kommentar unveraendert.
...
begin
  ...
  insert into public.invoices(
    ...
    -- issued_at traegt jetzt dasselbe gewaehlte Datum wie invoice_date, statt
    -- now(). Das ist die einzige Zeile, die sich gegenueber
    -- 2026-08-10_standalone_credit_notes.sql aendert.
    v_issued_on,v_issued_on::timestamp at time zone 'UTC',p_contract_id,
...
comment on function public.admin_create_standalone_credit_note_v1(text,numeric,text,date,uuid,uuid,text,text) is
  'Kulanzgutschrift ohne Bezugsrechnung. Nutzt Nummernkreis, Ablage, Idempotenz und Pruefprotokoll der rechnungsgebundenen Gutschrift. Nur ueber service_role (admin-mutate, Capability billing:write). issued_at = invoice_date = v_issued_on seit 20260811210000 (vorher issued_at faelschlich now()).';
