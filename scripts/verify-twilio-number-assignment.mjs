...
const files={index:'admin-panel/index.html',admin:'admin-panel/netlify/functions/admin-twilio-number-assignment.js',runtime:'admin-panel/shared/admin-runtime-twilio-number-assignment.js',router:'customer-dashboard/netlify/functions/twilio-inbound-router.js',migration:'supabase/migrations/2026-08-01_twilio_number_assignments.sql',correction:'supabase/migrations/2026-08-01_fix_twilio_assignment_customer_schema.sql',loader:'admin-panel/shared/offer-brand.js',customerUpdate:'admin-panel/netlify/functions/admin-customer-update.js'};
...
for(const token of ['Nummer ansehen / ändern',"step==='Voxera number assigned'",'openTwilioNumberAssignment'])if(!source.index.includes(token))failures.push('Completed number management entry missing: '+token);
for(const token of ['openTwilioNumberAssignment','nummer eintragen','data-assign','data-confirm-approve','data-status','Twilio-Zuordnung wird geprüft',"e.key==='Escape'"])if(!source.runtime.toLowerCase().includes(token.toLowerCase()))failures.push('Runtime missing: '+token);
