...
// Cross-page actions must use the canonical assistant router and load the
// selected view through its canonical owner.
const navigation = sourceFor('customer-dashboard/shared/customer-runtime-unified-navigation.js');
assert.match(
  navigation,
  /document\.addEventListener\('click',[\s\S]*businessProfileShortcut[\s\S]*preventDefault\(\)[\s\S]*stopPropagation\(\)[\s\S]*showAssistantView\('business', true\)[\s\S]*true\)/,
...
assert.match(assistant, /root\.vxShowAssistantView\?\.\('business', true\)/);
