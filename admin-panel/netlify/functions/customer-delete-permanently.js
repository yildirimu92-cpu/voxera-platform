...
exports.handler = async (event) => {
  ...
  const steps = [];
  // Lookup 1: via users-Tabelle (customer_id → auth user id)
  ...
  // Lookup 2: Fallback via customers.auth_user_id (wird seit neuestem gesetzt)
  let authUserId = userRow?.id || null;
  if (!authUserId) {
    ...
    if (custRow?.auth_user_id) {
      authUserId = custRow.auth_user_id;
    } else if (custRow?.email) {
      // Lookup 3: Fallback via E-Mail in auth.users
      const { data: { users: authUsers } } = await sbAdmin.auth.admin.listUsers({ perPage: 1000 });
      const found = (authUsers || []).find(u => u.email?.toLowerCase() === custRow.email.toLowerCase());
