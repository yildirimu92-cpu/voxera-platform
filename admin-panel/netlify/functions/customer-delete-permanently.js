 ...
 *   5a. customer_tasks     (FK → customers, ehem. cases · Customer Dashboard)
 *   5b. voxera_cases       (FK → customers, ehem. cases · Admin Portal)
 ...
 *
 * Hinweis zur Cases-Migration (April 2026):
 *   Die alte `cases` Tabelle wurde aufgesplittet in `customer_tasks` (Customer
 *   Dashboard) und `voxera_cases` (Admin Portal). Beide haben FK auf customers
 *   und müssen beim Hard-Delete entfernt werden.
 *
 ...
 * Ergebnis: users, subscriptions, contracts, onboarding, customer_tasks,
 *           voxera_cases, invoices, calls, offers
 *
...
async function permanentlyDeleteCustomer(sbAdmin, customerId) {
  ...
  try {
    ...
    // ── 5a. customer_tasks (migrated from `cases`, owned by customer dashboard) ─
    const { error: customerTasksErr } = await sbAdmin
      .from('customer_tasks')
      .delete()
      .eq('customer_id', customerId);
    if (customerTasksErr) throw new Error('customer_tasks löschen fehlgeschlagen: ' + customerTasksErr.message);
    steps.push('delete_customer_tasks');

    // ── 5b. voxera_cases (migrated from `cases`, owned by admin portal) ──────
    const { error: voxeraCasesErr } = await sbAdmin
      .from('voxera_cases')
      .delete()
      .eq('customer_id', customerId);
    if (voxeraCasesErr) throw new Error('voxera_cases löschen fehlgeschlagen: ' + voxeraCasesErr.message);
    steps.push('delete_voxera_cases');
