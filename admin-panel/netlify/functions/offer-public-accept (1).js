  ...
  try {
    ...
    const accepted = await acceptOfferAndEnsureContract({
      ...
      acceptanceMeta: {
        ...
        agb_version:  body.agb_version  || null,
        avv_accepted: body.avv_accepted === true,
        avv_version:  body.avv_version  || null,
        dse_accepted: body.dse_accepted === true,
        dse_version:  body.dse_version  || null,
      }
    });

    // Signatur in contracts speichern
    if (accepted.contractId && signatureData) {
      await sbAdmin
        .from('contracts')
        .update({ signature_data: signatureData, updated_at: nowIso })
        .eq('id', accepted.contractId);
    }
