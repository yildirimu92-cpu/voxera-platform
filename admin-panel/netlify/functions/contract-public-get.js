...
exports.handler = async (event) => {
  ...
  // Verknüpfte Offerte laden (für Kundensignatur + Legal-Versionen)
  let offer = null;
  if (contract.offer_id) {
    const { data: offerData } = await sbAdmin
      .from('offers')
      .select('accepted_at, accepted_by_name, accepted_by_email, accepted_ip, signature_data, agb_version, avv_version, dse_version, agb_accepted, avv_accepted, dse_accepted')
