...
exports.handler = async (event) => {
  ...
  // Verknüpfte Offerte laden (Legal-Versionen + Leistungsumfang)
  let offer = null;
  if (contract.offer_id) {
    const { data: offerData } = await sbAdmin
      .from('offers')
      .select('accepted_at, accepted_by_name, accepted_by_email, accepted_ip, agb_version, avv_version, dse_version, agb_accepted, avv_accepted, dse_accepted, included_minutes, extra_per_minute, number_count, add_ons, plan, billing_cycle, setup_fee, monthly_price, yearly_price, duration_months, starts_at')
