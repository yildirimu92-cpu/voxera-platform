...
function buildDerivedInvoiceItem(invoice, context) {
  ...
  if (type === 'setup_fee') title = 'Einrichtung und Inbetriebnahme Voxera';
    ...
    title = `${planLabel ? `Voxera ${planLabel}` : 'Voxera'} – ${cycle === 'yearly' ? 'Jahresabonnement' : 'Monatsabonnement'}`;
  } else if (compactText(invoice.notes)) {
    title = compactText(invoice.notes).slice(0, 90);
  }
  ...
  const includedMinutes = numberOr(
...
async function resolveInvoiceItems(sbAdmin, invoice, databaseItems) {
  const storedItems = (Array.isArray(databaseItems) ? databaseItems : [])
    .map(normalizeInvoiceItem)
    .filter((item) => item.title || item.line_total !== 0);
  ...
  const update = await sbAdmin
    .from('invoices')
    .update({ line_items_snapshot: derivedItems, updated_at: now })
    .eq('id', invoice.id)
    .select('*')
    .single();
