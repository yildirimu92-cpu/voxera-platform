...
async function fetchAvvPdfBuffer(sbAdmin) {
  const bucket = process.env.AVV_PDF_BUCKET || 'legal';
  const path   = process.env.AVV_PDF_PATH   || 'documents/avv-voxera.pdf';
  try {
    const { data, error } = await sbAdmin.storage.from(bucket).download(path);
    if (error || !data) {
      console.warn('fetchAvvPdfBuffer: PDF nicht gefunden oder Fehler:', error?.message || 'no data');
      return null;
    }
    const arrayBuffer = await data.arrayBuffer();
    return Buffer.from(arrayBuffer);
  } catch (err) {
    console.warn('fetchAvvPdfBuffer: Fehler beim Abrufen des AVV-PDFs:', err.message || String(err));
    return null; // Kein Hard-Fail — E-Mail wird ohne Anhang gesendet
  }
}

async function sendOfferEmail({ to, offer, subject, acceptanceUrl, pdfUrl, sbAdmin }) {
  ...
  const avvBuffer = await fetchAvvPdfBuffer(sbAdmin);
  const attachments = avvBuffer
    ? [{ filename: 'Voxera-AVV-v1.0.pdf', content: avvBuffer, contentType: 'application/pdf' }]
    : [];
  await transporter.sendMail({ from, to, subject: built.subject, html: built.html, attachments });
...
exports.handler = async (event) => {
  ...
  try {
    const deliveredSubject = await sendOfferEmail({ to: sentToEmail, offer, subject, acceptanceUrl, pdfUrl, sbAdmin });
