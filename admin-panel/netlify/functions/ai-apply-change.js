...
exports.handler = async (event) => {
  ...
  if (!customer_id || !request_message) {
    return { statusCode: 400, body: JSON.stringify({ error: 'customer_id and request_message required' }) };
  }
...
Generiere die notwendigen Feldänderungen als JSON.`;
  ...
  try {
    ...
    return { statusCode: 500, body: JSON.stringify({ error: 'AI generation failed: ' + e.message }) };
  }
