...
async function recordOfferEvent(sbAdmin, payload) {
  ...
  const ALLOWED_ACTOR_TYPES = new Set(['admin', 'customer', 'system']);
  const rawActorType = payload?.actor_type || null;
  const actorType = rawActorType && ALLOWED_ACTOR_TYPES.has(rawActorType) ? rawActorType : (rawActorType === 'public' ? 'customer' : null);
