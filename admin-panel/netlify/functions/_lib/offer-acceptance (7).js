...
async function acceptOfferAndEnsureContract({ sbAdmin, offer, nowIso, acceptanceMeta = null, allowContractFailure = false }) {
  ...
  try {
    ...
    // Signatur des Kunden im Vertrag speichern
    if (contractId && acceptanceMeta?.signature_data) {
      await sbAdmin
        .from('contracts')
        .update({
          signature_data:    acceptanceMeta.signature_data,
          accepted_by_name:  acceptanceMeta.accepted_by_name || null,
          billing_cycle:     offerWithCustomer.billing_cycle || null,
          setup_fee_amount:  Number(offerWithCustomer.setup_fee || 0),
          recurring_amount_monthly: Number(offerWithCustomer.monthly_price || 0),
          recurring_amount_yearly:  Number(offerWithCustomer.yearly_price || 0),
          included_minutes:  offerWithCustomer.included_minutes || null,
          overage_rate_per_minute: Number(offerWithCustomer.extra_per_minute || 0) || null,
          updated_at: nowIso
        })
        .eq('id', contractId);
    }
