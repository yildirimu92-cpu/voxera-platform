...
exports.handler = async (event) => {
  ...
  const fromStatus = normalizeCustomerStatus(customer.status);
  const toStatus = normalizeCustomerStatus('deleted');
  try {
    assertCustomerTransition(fromStatus, toStatus);
  } catch (e) {
  ...
  const { data, error } = await sbAdmin
    ...
    .select('*')
    .single();
  if (error) return response(500, { error: 'Kunde konnte nicht archiviert werden.', details: error.message });
