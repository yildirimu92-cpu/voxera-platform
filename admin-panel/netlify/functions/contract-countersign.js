...
exports.handler = async (event) => {
  ...
  const baseUrl = process.env.URL || process.env.PUBLIC_BASE_URL || 'https://voxera.ch';
  const signedPageUrl = `${baseUrl}/admin-panel/contract-signed.html?token=${publicToken}`;

  // Make.com Webhook für Vertragskopie triggern (immer, unabhängig von customerEmail)
  const mailWebhook = process.env.MAKE_COUNTERSIGN_WEBHOOK || process.env.MAKE_MAIL_WEBHOOK;
  if (mailWebhook) {
    try {
      const webhookRes = await fetch(mailWebhook, {
        method: 'POST',
      ...
      console.log('contract-countersign: webhook response status:', webhookRes.status);
    } catch (mailErr) {
      console.warn('contract-countersign: Webhook fehlgeschlagen:', mailErr.message);
    }
  } else {
    console.warn('contract-countersign: MAKE_COUNTERSIGN_WEBHOOK nicht konfiguriert.');
  }
