...
exports.handler = async (event) => {
  ...
  if (action === 'create_overage_invoice') {
    const overageMinutes = Math.max(0, Math.floor(Number(body.overage_minutes || 0)));
    const overageRate    = Math.max(0, Number(body.overage_rate || 0));
    const periodMonth    = String(body.period_month || '').trim(); // 'YYYY-MM'
    if (overageMinutes <= 0) {
      return response(400, { error: 'overage_minutes muss > 0 sein.' });
    }
    if (overageRate <= 0) {
      return response(400, { error: 'overage_rate muss > 0 sein.' });
    }
    if (!/^\d{4}-\d{2}$/.test(periodMonth)) {
      return response(400, { error: 'period_month muss YYYY-MM sein.' });
    }
    const contractId = String(body.contract_id || '').trim() || null;
    // Idempotency: one overage invoice per customer + month
    const externalReference = `overage:${customerId}:${periodMonth}`;
    const { data: existing, error: existingErr } = await sbAdmin
      ...
      .maybeSingle();
    if (existingErr) {
      return response(500, { error: 'Idempotency lookup failed', details: existingErr.message });
    }
    if (existing) {
      return response(200, {
        success: true,
    ...
    const periodStart = new Date(Date.UTC(Number(yearStr), Number(monthStr) - 1, 1)).toISOString();
    const periodEnd   = new Date(Date.UTC(Number(yearStr), Number(monthStr), 1)).toISOString();
    const nowIso = new Date().toISOString();
    const payload = {
      invoice_number: await sbAdmin.rpc('next_invoice_number_v1').then((r) => r.data),
      customer_id: customerId,
