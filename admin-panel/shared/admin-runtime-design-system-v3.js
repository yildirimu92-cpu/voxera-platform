(function (w) {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .customer-workspace-section,.offer-card,.offer-header-card,.vx-payment-account-card{
        background:var(--vx3-surface)!important;
        border:1px solid var(--vx3-line)!important;
        border-radius:var(--vx3-radius)!important;
        box-shadow:var(--vx3-shadow)!important;
        overflow:hidden;
      }

      .card:hover,.settings-card:hover,.profile-card:hover,.customer-section-shell:hover{
        box-shadow:var(--vx3-shadow-hover)!important;
      }
      ...
      .card-header,.settings-card-header,.bf-panel-header,.vx-unified-head{
        ...
        border-bottom:1px solid var(--vx3-line)!important;
        color:var(--vx3-ink)!important;
      }
      ...
      .bf-panel-header h2,.bf-panel-header h3,.vx-unified-head h2,.vx-unified-head h3{
        color:var(--vx3-ink)!important;font-size:16px!important;font-weight:760!important;
      ...
      input:not([type=checkbox]):not([type=radio]),select,textarea,.input,.select{
        ...
        border:1px solid var(--vx3-line-strong)!important;
        border-radius:11px!important;
        background:#fff!important;
        color:var(--vx3-ink)!important;
        font-size:14px!important;
        box-shadow:inset 0 1px 0 rgba(16,33,61,.02)!important;
      }
      ...
      .btn,button.btn,a.btn{
        min-height:42px!important;padding:9px 16px!important;border-radius:11px!important;
        font-size:13px!important;font-weight:720!important;box-shadow:none!important;
      }
      ...
      .toolbar,.filter-bar,.vx-toolbar{
        padding:12px!important;border-radius:14px!important;background:#fff!important;
        border:1px solid var(--vx3-line)!important;box-shadow:var(--vx3-shadow)!important;
      }

      .module-tabs,.bf-tabs,.segmented,.tab-bar{
        background:#EAF0F7!important;border:1px solid #DDE5EF!important;border-radius:13px!important;padding:5px!important;
      }
      ...
      .module-tab.active,.bf-tab-active,.segmented>button.active,.tab-bar>button.active{
        background:#fff!important;color:var(--vx3-ink)!important;box-shadow:0 3px 10px rgba(16,33,61,.09)!important;
      }
      ...
      tbody td{border-bottom:1px solid #EDF1F6!important}
      tbody tr:hover{background:#FAFCFF!important}

      .badge,[class*=badge],.status-pill{border-radius:999px!important;font-weight:720!important}

      /* Settings / package configuration */
      #section-settings .grid-4,#section-settings [style*="grid-template-columns: repeat(4"],#section-settings [style*="grid-template-columns:repeat(4"]{
        gap:16px!important;align-items:start!important;
      }
      #section-settings .grid-4>*,#section-settings [style*="grid-template-columns: repeat(4"]>*{
        border:1px solid var(--vx3-line)!important;border-radius:16px!important;background:#fff!important;
        box-shadow:0 8px 24px rgba(16,33,61,.045)!important;padding:18px!important;
      }
      ...
      /* Turn long package forms into visually separated sections */
      #section-settings .grid-4>* label:first-of-type,#section-settings .settings-grid>* label:first-of-type{margin-top:0!important}
      #section-settings .grid-4>* hr,#section-settings .settings-grid>* hr{border:0!important;border-top:1px solid var(--vx3-line)!important;margin:18px 0!important}

      /* Customer cards */
      #section-customers .customer-card,#section-customers .customer-row-card{
        border:1px solid var(--vx3-line)!important;border-radius:18px!important;box-shadow:var(--vx3-shadow)!important;overflow:hidden!important;
      }
      #section-customers .customer-card-header,#section-customers .customer-row-card>header{
        padding:18px 20px!important;background:var(--vx3-navy)!important;color:#fff!important;
      }
      ...
      /* Motion */
      .card,.settings-card,.profile-card,.customer-section-shell,.btn,input,select,textarea{
      ...
      @media(max-width:760px){
        :root{--vx3-radius:14px}
        .content{padding:14px 10px 32px!important}
  ...
  function markPaymentCardAsReference() {
    ...
    const heading = headings.find(node => /Zahlungskonto\s*&\s*QR-Rechnung/i.test(node.textContent || ''));
    if (!heading) return;
    const card = heading.closest('.card,.settings-card,.bf-panel,section,article,div');
    if (card) card.classList.add('vx-payment-account-card');
  }

  function tick() {
    addStyles();
    markPaymentCardAsReference();
  }
